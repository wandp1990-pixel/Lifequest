...
export async function initDbSchemaOnly() {
  ...
  try { await db.execute("ALTER TABLE character ADD COLUMN pity_count INTEGER DEFAULT 0") } catch {}
}
...
export async function initDb() {
  ...
  await runMigration("item_balance_v1_revert", async () => {
    await db.execute("UPDATE item_grade_table SET sub_count='0~1' WHERE grade='B'")
    await db.execute("UPDATE item_grade_table SET stat_max=10 WHERE grade='C'")
    await db.execute("UPDATE item_ability_pool SET base_value=50, effect='최대HP +50' WHERE name='HP증가'")
    await db.execute("UPDATE item_ability_pool SET base_value=5, effect='물리방어 +5' WHERE name='방어력'")
    await db.execute("UPDATE battle_config SET config_value='20' WHERE config_key='vit_to_max_hp'")
  ...
  await runMigration("gacha_pity_v1", async () => {
    try { await db.execute("ALTER TABLE character ADD COLUMN pity_count INTEGER DEFAULT 0") } catch {}
